  ...
  link.addEventListener("click", (event) => {
    ...
    if (targetSection) {
      const targetPosition =
        targetSection.getBoundingClientRect().top + window.pageYOffset - 150;
      window.scrollTo({
        top: targetPosition,
        behavior: "smooth",
      });
...
dropdownItems.forEach((item) => {
  item.addEventListener("click", () => {
    ...
    switch (category) {
      case "Числовий":
        document
          .querySelectorAll(".numbers_sections")
          .forEach((section) => (section.style.display = "block"));

        document
          .querySelectorAll(".games_sections")
          .forEach((section) => (section.style.display = "none"));
        document
          .querySelector(".users_sections")
          .forEach((section) => (section.style.display = "none"));
        break;
      case "Ігровий":
        document
        ...
        document
          ...
          .forEach((section) => (section.style.display = "block"));
        document
          .querySelectorAll(".users_sections")
          .forEach((section) => (section.style.display = "none"));
        break;
      case "Ознайомчий":
        document
...
const elements = [
  ...
  ...document.querySelectorAll(".footer__contacts-text"),
];
...
const headerLinks = document.querySelectorAll('.dropdown-item');
...
const toggleTheme = () => {
  ...
  footerLinks.forEach((link) => {
    link.style.color = checkbox.checked ? "dark" : "light";
  });
  ...
  arrowNext.style.backgroundImage = isDark
    ? "url(img/arrow-next-dark.svg)"
    : "url(img/arrow-next.svg)";
  arrowPrev.style.backgroundImage = isDark
    ? "url(img/arrow-prew-dark.svg)"
    : "url(img/arrow-prew.svg)";
